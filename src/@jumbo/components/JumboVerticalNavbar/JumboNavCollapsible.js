...
const JumboNavCollapsible = ({ item, translate, classNames }) => {
  const location = useLocation();
  const [open, setOpen] = React.useState(isUrlInChildren(item, location.pathname));
  ...
  return (
    <React.Fragment>
      ...
      </ListItemButton>
      {
        ...
        <Collapse component={"ul"} in={open} timeout="auto" unmountOnExit>
          <List disablePadding>
            {
              subMenus.map((child, index) => {
                const eventKey = new Date().valueOf();
                return <JumboNavIdentifier classNames="sub-submenu-main" item={child} key={`${eventKey}${index}`} isNested={true} />
              })
            }
          </List>
        </Collapse>
