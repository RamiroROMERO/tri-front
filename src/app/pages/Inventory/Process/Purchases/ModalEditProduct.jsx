...
export const ModalEditProduct = ({setOpen, data, t}) => {
  const {currentItemDeta, sweetAlerts, setLoading, purchaseDetail, onBulkFormIndex, fnUpdateTotals} = data;
  ...
  const fnSave = ()=>{
    ...
    setLoading(true);
    request.PUT(`/inventory/purchaseDetails/${id}`, detailPurchase, resp=>{
      const {messages} = resp;
    ...
    const sumDiscount = purchaseDetail.map(item => validFloat(item.discountValue)).reduce((prev, curr) => prev + curr, 0);
    
    const sumTaxes = purchaseDetail.map(item => validFloat(item.taxValue)).reduce((prev, curr) => prev + curr, 0);
    const sumTotal = purchaseDetail.map(item => validFloat(item.total)).reduce((prev, curr) => prev + curr, 0);
