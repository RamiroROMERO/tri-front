...
import { Add, Delete, Done, Edit, List } from '@mui/icons-material'
...
export const usePurchases = ({ setLoading, sweetAlerts }) => {
  ...
  const fnGetData = () => {
    setLoading(true);
    request.GET(`/inventory/purchases`, (resp) => {
      const data = resp.data.map((item) => {
        item.customerName = item.invCustomer ? item.invCustomer.name : ''
        item.projectName = item.invProject ? item.invProject.code + ' | ' + item.invProject.name : ''
        item.providerName = item.invProvider ? item.invProvider.name : ''
        item.statusIcon = item.status === 2 ? "Closed" : "Pending"
        return item;
      });
      setTableData(data);
      setLoading(false);
  ...
  const fnDeleteDocto = (item) => {
    if (item.status === 2) {
      sweetAlerts('error', t("alertMessages.error.purchaseClosed"));
      return;
    }
    setCurrentItem(item);
  ...
  const fnOkDelete = () => {
    setLoading(true);
    request.DELETE(`/inventory/purchases/${currentItem.id}`, resp => {
      const { messages } = resp;
  ...
  const fnOkCloseDocto = () => {
    setLoading(true);
    request.PUT(`/inventory/purchases/close/${currentItem.id}`, {}, resp => {
      const { messages } = resp;
      if (messages?.length > 0) {
        messages.map(elem => {
          sweetAlerts(elem.type, t(elem.message));
  ...
  const table = {
    title: t("table.purchases.title"),
    columns: [
      { field: 'noPurchase', headerName: t("table.common.num"), flex: 0.3 },
      {
        field: 'dateIn',
