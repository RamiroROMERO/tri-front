...
export const useStores = ({ setLoading, sweetAlerts, t }) => {
  ...
  const fnOkDelete = () => {
    ...
    setLoading(true);
    request.DELETE(`/inventory/stores/${currentItem.id}`, resp => {
      const { messages } = resp;
  ...
  const table = {
    title: t("table.stores.title"),
    columns: [
      { field: 'name', headerName: t("table.common.name"), flex: 1 },
      { field: 'location', headerName: t("table.common.location"), flex: 1 },
      { field: 'qtyProducts', headerName: t("table.common.qtyProducts"), flex: 0.5 },
      {
        field: 'statusIcon',
