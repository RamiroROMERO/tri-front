...
export const useMeasurementUnits = ({ setLoading, sweetAlerts, t }) => {
  ...
  const fnGetData = () => {
    setLoading(true);
    request.GET('/inventory/measureUnits', (resp) => {
  ...
  const fnOkDelete = () => {
    ...
    setLoading(true);
    request.DELETE(`/inventory/measureUnits/${currentItem.id}`, resp => {
      const { messages } = resp;
  ...
  const table = {
    title: t("table.measurementUnits.title"),
    columns: [
      { field: 'code', headerName: t("table.common.code"), flex: 0.5 },
