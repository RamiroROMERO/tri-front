...
export const useProductTypes = ({ setLoading, sweetAlerts }) => {
  ...
  const fnGetData = () => {
    setLoading(true);
    request.GET('/inventory/classifications', (resp) => {
  ...
  const fnOkDelete = () => {
    ...
    setLoading(true);
    request.DELETE(`/inventory/classifications/${currentItem.id}`, resp => {
      const { messages } = resp;
  ...
  const table = {
    title: t("table.productTypes.title"),
    columns: [
