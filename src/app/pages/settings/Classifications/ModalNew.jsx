...
const ModalNew = ({setOpen, data, t}) => {
  ...
  const {noOrder, name, rateByHour, rateByInvoice, status} = formState;

  const {noOrderValid, nameValid} = formValidation;
      ...
      <DialogContent dividers>
        <Grid container spacing={3} direction='row'>
          <Grid item xs={12} sm={4}>
            <InputBox
              label={t('table.classifications.order')}
              name='noOrder'
              value={noOrder}
              onChange={onInputChange}
              error={(sendForm&&noOrderValid)?true:false}
              helperText={(sendForm &&!isFormValid)&&noOrderValid}
              required
            />
          </Grid>
          ...
          <Grid item xs={12} sm={6}>
            <InputBox
              label={t('table.classifications.rateByHour')}
              name='rateByHour'
              value={rateByHour}
              onChange={onInputChange}
            />
          ...
          <Grid item xs={12} sm={6}>
            <InputBox
              label={t('table.classifications.rateByInvoice')}
              name='rateByInvoice'
              value={rateByInvoice}
              onChange={onInputChange}
            />
