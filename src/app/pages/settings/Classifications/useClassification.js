...
export const useClassification = ({setLoading, sweetAlerts, controlAdmin}) => {
  ...
  const fnGetData = ()=>{
    ...
    request.GET('/classifications', (resp)=>{
      const data= resp.classifications.map((item)=>{
  ...
  const fnNew = ()=>{
    ...
    setCurrentItem({});
    setTitleModalNew("dialog.classifications.title");
  ...
  const fnEdit = (item)=>{
    ...
    setCurrentItem(item);
    setTitleModalNew("dialog.classifications.edit.title");
    setOpenModalNew(true);
  }
  ...
  const fnOkDelete = ()=>{
    ...
    setLoading(true);
    request.DELETE(`/classifications`, resp=>{
      const {messages} = resp;
  ...
  const table = {
    ...
    columns: [
      {field: 'noOrder', headerName: t('table.classifications.order'), flex: 0.3 },
      {field: 'name', headerName: t('table.common.name'), flex: 1.2 },
      {field: 'rateByHour', headerName: t('table.classifications.rateByHour'), type: 'number', flex: 0.8},
      {field: 'rateByInvoice', headerName: t('table.classifications.rateByInvoice'), type: 'number', flex: 0.8},
