...
export const useModalContacts = ({t, setLoading, sweetAlerts, customerId}) => {
  ...
  const fnGetData = ()=>{
    setLoading(true);
    request.GET(`/contacts?customerId=${customerId}`, (resp)=>{
      const data = resp.contacts.map((item)=>{
  ...
  const fnSave = () =>{
    ...
    setLoading(true);
    if(validInt(id)===0){
      request.POST('/contacts', newData, resp=>{
        const {messages} = resp;
        if(messages?.length>0){
          messages.map(elem=>{
            sweetAlerts(elem.type, t(elem.message));
      ...
      request.PUT('/contacts', newData, resp=>{
        const {messages} = resp;
        if(messages?.length>0){
          messages.map(elem=>{
            sweetAlerts(elem.type, t(elem.message));
