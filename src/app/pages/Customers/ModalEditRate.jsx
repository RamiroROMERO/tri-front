...
export const ModalEditRate = ({setOpen, data, t}) => {
  const {sweetAlerts, setLoading, currentItemRate, customerId, fnGetData} = data;
  const employeeName = currentItemRate?.name || '';

  const {formState, formValidation, isFormValid, sendForm, onInputChange, onPercOtPayrollChange, onPercOtInvoiceChange, fnSave} = useModalEditRate({t, sweetAlerts, setLoading, currentItemRate, customerId, fnGetData, setOpen});

  const {rate, percOtPayroll, rateOvertime, rateInvoice, percOtInvoice, rateInvoiceOvertime, payPerdiem, invoicePerdiem, fixedPerdiem, fixedPerdiem2, status} = formState;

  const {rateValid, rateInvoiceValid} = formValidation;
  ...
  <>
    <DialogContent dividers>
      ...
      <Grid container spacing={3} direction='row' sx={{mt: 0}}>
